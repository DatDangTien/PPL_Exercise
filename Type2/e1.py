...
# Rewrite the body of the methods in class StaticCheck to infer the type of identifiers
#   and check the following type constraints:
...
#   if the Id is not in the declarations, exception UndeclaredIdentifier should 
#     be raised together with the name of the Id, or
#   If the Id cannot be inferred in the first usage, exception TypeCannotBeInferred
#   should be raised together with the assignment statement which contains the 
#   type-unresolved identifier.

# For static referencing environment, this language applies the scope rules of 
#   block-structured programming language. When there is a declaration duplication 
#   of a name in a scope, exception Redeclared should be raised together with the 
#   second declaration.
...
class StaticCheck(Visitor):
    ...
    def visitProgram(self,ctx:Program,o:object):
        ...
        for decl in ctx.decl:
            o[0].append(self.visit(decl,o[0]))
    ...
    def visitBinOp(self,ctx:BinOp,o:object):
        ...
        #update_typ for Id
        if not typ_e1:
            typ_e1 = Utils.infer(o, ctx.e1.name, accept_typ)
        if not typ_e2:
            typ_e2 = Utils.infer(o, ctx.e2.name, accept_typ)
    ...
    def visitUnOp(self,ctx:UnOp,o:object):
        ...
        if typ_e:    
            ...
            #update_typ for Id
            Utils.infer(o, ctx.e.name, accept_typ)
            return return_typ
